...
export function extractStrings(
  ...
  function traverse(
    node: SyntaxNode,
    previousNode: SyntaxNode | null = null,
  ): void {
    if (node.type === typeToMatch) {
      ...
      if (
        ...
        previousNode.text.includes("translators:")
      ) {
        comments = previousNode.text.trim();
      }

      const type =
        i18nFunctions[fn.text as keyof typeof i18nFunctions] ?? "text_domain";
...
function getJsonComment(
  ...
  const comments =
    type === "block.json" ? blockJsonComments : themeJsonComments;
        ...
        return (
          Object.entries(parsed as Record<string, string | string[]>)
            ...
            .map(([key, jsonData]) => {
              ...
              if (!jsonData) {
                args.stats?.bar.increment(0, {
                  filename: `Skipping ${key} in ${args.filepath} as ${filename} ... cannot parse data`,
                });
